...
const EmpanelmentHierarchyView = (props) => {
    ...
    const gethierarchyEmpanelmentData = () => {
        if (startDate && endDate && endDate > startDate) {
            axiosPrivateCall.get(`/api/v1/BDE/gethierarchyEmpanelmentData?user_id=${userId}&start_date=${formateDate(startDate)}&end_date=${formateDate(endDate)}`)
                .then(res => {
                    setHierarchyEmpanelmentData(res.data)
                }).catch(e => console.log(e))
        }
        else {
            axiosPrivateCall.get(`/api/v1/BDE/getHierarchyEmpanelment?user_id=${userId}`).then(res => {
                setHierarchyEmpanelmentData(res.data)
            }).catch(e => console.log(e))
        }
    }
