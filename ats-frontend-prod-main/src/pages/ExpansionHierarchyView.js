...
const ExpansionHierarchyView = (props) => {
    ...
    const [hierarchyExpansionData, setHierarchyExpansionData] = useState([]);

    const gethierarchyExpansionData = () => {
        if (startDate && endDate && endDate > startDate) {
            axiosPrivateCall.get(`/api/v1/BDE/getHierarchyExpansion?user_id=${userId}&start_date=${formateDate(startDate)}&end_date=${formateDate(endDate)}`)
                .then(res => {
                    setHierarchyExpansionData(res.data)
                    console.log(res.data, 778)
                }).catch(e => console.log(e))
        }
        else {
            axiosPrivateCall.get(`/api/v1/BDE/getHierarchyExpansion?user_id=${userId}`).then(res => {
                setHierarchyExpansionData(res.data)
            }).catch(e => console.log(e))
        }
    }
        ...
        <div>
            <Modal id="Modal12" scrollableContentClassName={styles.adddemand_modal_scrollable_content} containerClassName={`${isModalShrunk ? styles.adddemand_modal_container_shrunk : styles.adddemand_modal_container}`}
                    ...
                    <div className={styles.header_content_container}>
                        <div className={styles.header_content_job_description_unassigned_save_container}>
                            <div className={styles.header_save_close_btns_container}>
                                Overall Expansion:  {hierarchyExpansionData?.expansion_count
                                }
                            </div>
                        </div>
                    </div>
                ...
                <div className={styles.main_information_container}>
                    <HierarchyCard name={hierarchyExpansionData?.current_user?.first_name} role={hierarchyExpansionData?.current_user?.role}
                        count={hierarchyExpansionData?.expansion_count} countName={"Expansion"} type={'expansion'} ActiveuserId={userId}
                        startDate={startDate} endDate={endDate}
                    />
                    {hierarchyExpansionData?.subbordinates?.length ? <HierarchyArrow /> : ''}
                    {
                        hierarchyExpansionData?.subbordinates?.length ? hierarchyExpansionData.subbordinates.map((data, index) => {
                            return (
                                <div onClick={() => hierarchyCardClickHandler(data._id)}>
                                    <HierarchyCard name={`${data.first_name + ' ' + data.last_name}`} role={data.role}
                                        type={'expansion'} userId={data._id} startDate={startDate} endDate={endDate}
                                    />
                                    {hierarchyExpansionData?.subbordinates?.length !== index + 1 ? <HierarchyArrow /> : ''}
                                </div>
                            )
                        }) : ''
                    }
...
export default ExpansionHierarchyView;
